...
/**
 * Pure SVG art for the identity companions. `companionSvg(type, stage, strength, mood)` returns a
 ...
 *
 * `mood` swaps facial expressions (`happy` smiles, `sad` frowns, `purr` closes the cat's eyes);
 * `neutral` is the resting look. Whole-body motion (cheer/droop) and the tap reactions are driven
 * by CSS in the companion component — the art only exposes the pieces those animations target:
 * a `wave-arm` group on the tree and `wingL`/`wingR` groups on the adult butterfly.
 *
 * Coordinates use a 0 0 100 100 viewBox (except Sprite, which is 0 0 140 150), with the
 * ground line around y = 88–91.
 */

/** Facial expression for a companion. `purr` is a cat-only content state used during its tap. */
export type CompanionMood = 'neutral' | 'happy' | 'sad' | 'purr';
...
/** A group the CSS wave animation rotates for the tree's tap reaction. */
const waveArm = (ox: number, oy: number, inner: string) =>
  `<g class="wave-arm" style="transform-box:view-box;transform-origin:${ox}px ${oy}px">${inner}</g>`;
/** A mouth curve whose bend follows the mood. `w` is the mouth width, `sw` the stroke width. */
const mouthCurve = (x: number, y: number, w: number, mood: CompanionMood, sw: number) => {
  let d: string;
  if (mood === 'happy' || mood === 'purr') d = `M${x} ${y - 0.5} q${w / 2} ${w * 1.05} ${w} 0`;
  else if (mood === 'sad') d = `M${x} ${y + 1.6} q${w / 2} ${-w * 0.75} ${w} 0`;
  else d = `M${x} ${y} q${w / 2} ${w * 0.7} ${w} 0`;
  return `<path d="${d}" stroke="${C.ink}" stroke-width="${sw}" fill="none" stroke-linecap="round"/>`;
};

// ---------------- TREE ----------------
function treeFace(cx: number, cy: number, s: number, mood: CompanionMood | 'sleep'): string {
  if (mood === 'sleep') {
  ...
  let brows = '';
  if (mood === 'sad') {
    brows = `<path d="M${cx - g - 3 * s} ${cy - 4.6 * s} l${4.2 * s} ${1.8 * s}" stroke="${C.ink}" stroke-width="${1.3 * s}" fill="none" stroke-linecap="round"/>` +
      `<path d="M${cx + g + 3 * s} ${cy - 4.6 * s} l${-4.2 * s} ${1.8 * s}" stroke="${C.ink}" stroke-width="${1.3 * s}" fill="none" stroke-linecap="round"/>`;
  }
  return `<ellipse cx="${cx - g}" cy="${cy}" rx="${er}" ry="${er * 1.15}" fill="#fff"/><ellipse cx="${cx + g}" cy="${cy}" rx="${er}" ry="${er * 1.15}" fill="#fff"/>` +
    ...
    brows + mouthCurve(cx - 3.6 * s, cy + 4.6 * s, 7.2 * s, mood, 1.5 * s);
}
...
function treeStages(mood: CompanionMood): string[] {
  const m0: CompanionMood | 'sleep' = mood === 'neutral' ? 'sleep' : mood;
  return [
    svg(shadow + acorn(50, 76, 12) + stem(50, 76 - 12 * 0.78, 12) + treeFace(50, 77, 1, m0)),
    svg(shadow + acorn(50, 76, 12) + sprout(50, 76 - 12 * 0.78 - 3) + treeFace(50, 78, 1, mood)),
    svg(shadow + `<rect x="44" y="58" width="12" height="30" rx="5" fill="${C.trunk}"/><circle cx="40" cy="46" r="12" fill="${C.leaf3}"/><circle cx="60" cy="46" r="12" fill="${C.leaf2}"/><circle cx="50" cy="40" r="16" fill="${C.leaf1}"/><path d="M44 73 q-4 0 -6 -3" stroke="${C.trunk}" stroke-width="2.4" fill="none" stroke-linecap="round"/><path d="M56 73 q4 0 6 -3" stroke="${C.trunk}" stroke-width="2.4" fill="none" stroke-linecap="round"/>` + treeFace(50, 70, 1.05, mood)),
    svg(shadow + `<rect x="42.5" y="54" width="15" height="34" rx="6" fill="${C.trunk}"/><circle cx="34" cy="46" r="14" fill="${C.leaf3}"/><circle cx="66" cy="46" r="14" fill="${C.leaf2}"/><circle cx="50" cy="46" r="15" fill="${C.leaf3}"/><circle cx="50" cy="32" r="17" fill="${C.leaf1}"/>` + treeArm(42.5, 70, 1, 2.8, 1) + waveArm(57, 68, treeArm(57.5, 70, -1, 2.8, 1)) + treeFace(50, 69, 1.15, mood)),
    svg(shadow + glowC(34) + `<path d="M40 88 Q38 66 43 56 L57 56 Q62 66 60 88 Z" fill="${C.trunk}"/><path d="M43 88 Q46 80 40 76 M57 88 Q54 80 60 76" stroke="${C.trunk}" stroke-width="3" fill="none" stroke-linecap="round"/><circle cx="30" cy="42" r="15" fill="${C.leaf3}"/><circle cx="70" cy="42" r="15" fill="${C.leaf2}"/><circle cx="40" cy="28" r="15" fill="${C.leaf3}"/><circle cx="60" cy="28" r="15" fill="${C.leaf2}"/><circle cx="50" cy="38" r="18" fill="${C.leaf1}"/><circle cx="50" cy="22" r="15" fill="${C.leaf1}"/><path d="M42 66 q-8 -1 -11 -7 q-1 -4 1 -8" stroke="${C.trunk}" stroke-width="3.2" fill="none" stroke-linecap="round"/>${leaf(31, 50, 1.4)}` + waveArm(58, 66, `<path d="M58 66 q8 -1 11 -7 q1 -4 -1 -8" stroke="${C.trunk}" stroke-width="3.2" fill="none" stroke-linecap="round"/>${leaf(69, 50, 1.4)}`) + `<ellipse cx="26" cy="52" rx="2.4" ry="3" fill="${C.nut}"/><ellipse cx="74" cy="52" rx="2.4" ry="3" fill="${C.nut}"/>` + treeFace(50, 68, 1.25, mood)),
  ];
}
...
const eyeClosed = (x: number, y: number, r: number) =>
  `<path d="M${x - r} ${y} q${r} ${r} ${r * 2} 0" stroke="${C.ink}" stroke-width="1.2" fill="none" stroke-linecap="round"/>`;
...
function catProfile(o: CatOpts, mood: CompanionMood): string {
  ...
  out += `<circle cx="${hx - o.headR * 0.55}" cy="${hy + o.headR * 0.25}" r="${o.headR * 0.42}" fill="${C.fur}"/>`;
  const nx = hx - o.headR * 0.92, ny = hy + o.headR * 0.12;
  out += `<circle cx="${nx}" cy="${ny}" r="${1.6 * s}" fill="${C.pink}"/>`;
  const ex = hx - o.headR * 0.05, ey = hy - o.headR * 0.05, er = 3 * s;
  out += mood === 'purr' ? eyeClosed(ex - er, ey, er) : o.adult ? eyeAlmond(ex, ey, er) : eyeOpen(ex, ey, er);
  if (mood === 'happy' || mood === 'purr') {
    out += `<path d="M${nx} ${ny + 2} q-2.6 3 -5.4 1.4 M${nx} ${ny + 2} q1.8 2.4 4 1.2" stroke="${C.furDark}" stroke-width="1.1" fill="none" stroke-linecap="round"/>`;
  } else if (mood === 'sad') {
    out += `<path d="M${nx} ${ny + 3.4} q-2.6 -2 -5.4 -0.4" stroke="${C.furDark}" stroke-width="1.1" fill="none" stroke-linecap="round"/>`;
  } else {
    out += `<path d="M${nx} ${ny + 2.6} q-2.4 1.6 -4.6 0.6" stroke="${C.furDark}" stroke-width="1.1" fill="none" stroke-linecap="round"/>`;
  }
...
function catNewborn(mood: CompanionMood): string {
  let face: string;
  if (mood === 'happy' || mood === 'purr') face = `<path d="M28 79 q2.4 2.4 4.8 0" stroke="${C.ink}" stroke-width="1.3" fill="none" stroke-linecap="round"/>`;
  else if (mood === 'sad') face = `<path d="M28 80 q2.4 -2 4.8 0" stroke="${C.ink}" stroke-width="1.3" fill="none" stroke-linecap="round"/>`;
  else face = `<path d="M28 79 q2.4 2.2 4.8 0" stroke="${C.ink}" stroke-width="1.3" fill="none" stroke-linecap="round"/>`;
  return svg(shadow +
...
function catStages(mood: CompanionMood): string[] {
  return [
    catNewborn(mood),
    catProfile({ s: 0.9, bodyLen: 27, bodyH: 20, legLen: 6, headR: 11, tailLen: 12, earH: 0.9 }, mood),
    catProfile({ s: 1.0, bodyLen: 34, bodyH: 21, legLen: 10, headR: 11.3, tailLen: 16, earH: 1.1 }, mood),
    catProfile({ s: 1.06, bodyLen: 41, bodyH: 22, legLen: 13.5, headR: 11, tailLen: 20, earH: 1.3 }, mood),
    catProfile({ s: 1.12, bodyLen: 46, bodyH: 22, legLen: 16.5, headR: 10.8, tailLen: 25, earH: 1.5, adult: true, tailUp: true, sparkle: true, glow: true }, mood),
  ];
}
...
/** The caterpillar's little face, whose smile follows the mood. `bx` is the left-eye x. */
function caterFace(bx: number, mood: CompanionMood): string {
  const by = 76;
  return `<path d="M${bx} ${by - 7} q-1 -6 -3 -7 M${bx + 6} ${by - 7} q1 -6 3 -7" stroke="${C.cat2}" stroke-width="1.4" fill="none" stroke-linecap="round"/>` +
    `<circle cx="${bx}" cy="${by}" r="1.5" fill="${C.ink}"/><circle cx="${bx + 5.5}" cy="${by}" r="1.5" fill="${C.ink}"/>` +
    mouthCurve(bx, by + 4.5, 6, mood, 1.3);
}
/** A group the CSS flutter animation flaps for the adult butterfly's tap reaction. */
const wingGroup = (side: 'L' | 'R', inner: string) =>
  `<g class="wing${side}" style="transform-box:view-box;transform-origin:50px 55px">${inner}</g>`;
function butterflyStages(mood: CompanionMood): string[] {
  return [
    ...
    svg(shadow + seg(38, 77, 8) + seg(50, 75, 8.5) + seg(62, 77, 9) + caterFace(59.5, mood) + `<circle cx="38" cy="85" r="1.3" fill="${C.cat2}"/><circle cx="50" cy="84" r="1.3" fill="${C.cat2}"/><circle cx="62" cy="86" r="1.3" fill="${C.cat2}"/>`),
    svg(shadow + seg(28, 78, 7.5) + seg(39, 75, 8.5) + seg(50, 74, 9) + seg(61, 75, 9.5) + seg(72, 77, 10) + caterFace(69.5, mood) + `<circle cx="30" cy="86" r="1.4" fill="${C.cat2}"/><circle cx="42" cy="85" r="1.4" fill="${C.cat2}"/><circle cx="54" cy="84" r="1.4" fill="${C.cat2}"/><circle cx="66" cy="85" r="1.4" fill="${C.cat2}"/>`),
    ...
    svg(`<circle cx="50" cy="54" r="30" fill="${C.gold}" opacity="0.16"/>` +
      wingGroup('L', `<path d="M49 50 C30 30 12 44 19 57 C25 68 44 61 49 55 Z" fill="${C.wing}" stroke="${C.edge}" stroke-width="2.2"/><path d="M49 57 C37 59 30 68 36 76 C42 82 49 71 49 63 Z" fill="${C.wing}" stroke="${C.edge}" stroke-width="2"/><circle cx="30" cy="48" r="2.4" fill="#fff"/><circle cx="40" cy="71" r="2" fill="#fff"/>`) +
      wingGroup('R', `<path d="M51 50 C70 30 88 44 81 57 C75 68 56 61 51 55 Z" fill="${C.wing}" stroke="${C.edge}" stroke-width="2.2"/><path d="M51 57 C63 59 70 68 64 76 C58 82 51 71 51 63 Z" fill="${C.wing}" stroke="${C.edge}" stroke-width="2"/><circle cx="70" cy="48" r="2.4" fill="#fff"/><circle cx="60" cy="71" r="2" fill="#fff"/>`) +
      `<ellipse cx="50" cy="56" rx="3.4" ry="16" fill="${C.edge}"/><circle cx="50" cy="40" r="3.6" fill="${C.edge}"/><path d="M50 37 q-6 -7 -11 -5 M50 37 q6 -7 11 -5" stroke="${C.edge}" stroke-width="1.6" fill="none" stroke-linecap="round"/><circle cx="39" cy="32" r="1.6" fill="${C.edge}"/><circle cx="61" cy="32" r="1.6" fill="${C.edge}"/>`),
  ];
}

// ---------------- SPRITE (legacy, theme-tinted) ----------------
function spriteSvg(stage: number, strength: number, mood: CompanionMood): string {
  ...
  if (stage >= 1) {
    ...
    let d: string, sw: number, extra = '';
    if (mood === 'happy' || mood === 'purr') { d = `M ${cx - mw} ${my} q ${mw} ${3 + t * 6} ${mw * 2} 0`; sw = 1.8; }
    else if (mood === 'sad') { d = `M ${cx - mw} ${my + 3} q ${mw} ${-4 - t * 3} ${mw * 2} 0`; sw = 1.8; }
    else { d = `M ${cx - mw} ${my} q ${mw} ${3 + t * 4} ${mw * 2} 0`; sw = 1.6; extra = ' opacity="0.8"'; }
    out += `<path d="${d}" style="stroke:var(--ink)" stroke-width="${sw}" fill="none" stroke-linecap="round"${extra}/>`;
  }
  return `<svg viewBox="0 0 140 150" role="img" xmlns="http://www.w3.org/2000/svg"><title>companion</title>${out}</svg>`;
}

export function companionSvg(type: CompanionType, stage: number, strength = 0, mood: CompanionMood = 'neutral'): string {
  const s = clampStage(stage);
  switch (type) {
    case 'Tree': return treeStages(mood)[s];
    case 'Cat': return catStages(mood)[s];
    case 'Butterfly': return butterflyStages(mood)[s];
    default: return spriteSvg(s, strength, mood);
  }
}
